...
import { CLASSROOMS, ACTIVITY_GROUPS, PRESET_SCHOOL_IMAGES } from '../data/mockData';
...
export const TeacherDashboard: React.FC<TeacherDashboardProps> = ({
  ...
  // Handle image files from upload or drag & drop
  ...
  const handleAddCustomUrl = () => {
    if (imageUrlInput.trim() && !uploadedImages.includes(imageUrlInput.trim())) {
              ...
              >
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                  <div className="space-y-1.5 flex-1">
                    ...
                    {/* Image Attachment Preview for Teachers */}
                          ...
                          {ann.images.map((imgUrl, idx) => (
                            <div key={idx} className="relative group shrink-0">
                              <img
                                ...
                                className="w-14 h-14 object-cover rounded-xl border border-slate-200 shadow-2xs group-hover:scale-105 transition-transform"
                              />
                            </div>
                          ))}
                        </div>
            ...
            <form onSubmit={handleCreateSubmit} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                    <ImageIcon className="w-3.5 h-3.5 text-indigo-600" />
                    <span>3. รูปภาพประกอบประกาศ (Accompanying Photos)</span>
                  </label>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                    ...
                    {uploadedImages.length > 0 ? `แนบแล้ว ${uploadedImages.length} รูป` : 'ยังไม่ได้แนบรูป'}
                  </span>
                </div>
                ...
                {/* Drag and Drop Zone & Click Picker */}
                <div
                ...
                >
                  <div className="flex flex-col items-center justify-center gap-1.5 pointer-events-none">
                    ...
                    <p className="text-xs font-semibold text-slate-700">
                      ลากรูปภาพมาวางที่นี่ หรือ <span className="text-indigo-600 underline">คลิกเพื่อเลือกไฟล์</span>
                    </p>
                    <p className="text-[11px] text-slate-400">
                      รองรับไฟล์ภาพ JPG, PNG, WEBP, GIF (แนบได้หลายรูปพร้อมกัน)
                    </p>
                  </div>
                </div>

                {/* Quick Presets & URL link toggle */}
                <div className="pt-1 space-y-2">
                  <div className="flex flex-wrap items-center justify-between gap-1.5 text-xs text-slate-500">
                    <span className="font-medium flex items-center gap-1">
                      <Sparkles className="w-3.5 h-3.5 text-amber-500" />
                      <span>หรือเลือกภาพตัวอย่างสำเร็จรูปของโรงเรียน:</span>
                    </span>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span>{showUrlInput ? 'ซ่อนช่องใส่ URL' : '+ แนบผ่าน URL รูปภาพ'}</span>
                    </button>
                  </div>

                  {/* Preset Pills */}
                  <div className="flex flex-wrap gap-1.5">
                    {PRESET_SCHOOL_IMAGES.map((preset) => {
                      const isAdded = uploadedImages.includes(preset.url);
                      return (
                        <button
                          type="button"
                          key={preset.id}
                          ...
                          className={`text-xs px-2.5 py-1 rounded-lg border transition-all flex items-center gap-1.5 ${
                            isAdded
                              ...
                              : 'bg-white text-slate-700 border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/50 cursor-pointer shadow-2xs'
                          }`}
                        >
                          <span>{isAdded ? '✓' : '+'}</span>
                  ...
                  {showUrlInput && (
                    <div className="flex items-center gap-2 pt-1">
                      <input
                        type="url"
                        placeholder="https://example.com/school-event-banner.jpg"
                        value={imageUrlInput}
                        onChange={(e) => setImageUrlInput(e.target.value)}
                        className="flex-1 text-xs px-3 py-1.5 bg-white border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      />
                      <button
                        type="button"
                        onClick={handleAddCustomUrl}
                        disabled={!imageUrlInput.trim()}
                        className="text-xs font-semibold px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-lg transition-colors cursor-pointer"
                      >
                        เพิ่มรูป
                      </button>
                    </div>
                ...
                {uploadedImages.length > 0 && (
                  <div className="pt-2 border-t border-slate-200/80">
                    <span className="text-[11px] font-semibold text-slate-600 block mb-2">
                      รูปภาพที่แนบในประกาศนี้ ({uploadedImages.length} รูป):
                    </span>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                      {uploadedImages.map((imgUrl, index) => (
                        <div
                          key={index}
                          className="group relative rounded-xl overflow-hidden border border-slate-200 bg-slate-100 shadow-2xs aspect-video"
                        >
                          <img
                            src={imgUrl}
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                            <button
                              ...
                              title="ลบรูปภาพนี้"
                              className="w-8 h-8 rounded-full bg-rose-600 text-white flex items-center justify-center hover:bg-rose-700 shadow-md cursor-pointer transition-transform hover:scale-110"
                            >
                              <Trash2 className="w-4 h-4" />
                            </button>
                          </div>
                          <span className="absolute bottom-1 left-1.5 bg-black/70 text-white text-[10px] px-1.5 py-0.5 rounded font-medium">
                            รูปที่ {index + 1}
                          </span>
                        </div>
                      ))}
